...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard 'BUILD' '*/BUILD.bazel' '*.bzl' '*.BUILD' 'WORKSPACE' '*.bazel' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.js' '*.cjs' '*.mjs' '*.ts' '*.tsx' '*.mts' '*.cts' '*.json' '*.css' '*.html' '*.md' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.sql' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
  files=$(find "$@" -name '*.sql')
fi
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.py' '*.pyi' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.tf' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
  files=$(find "$@" -name '*.tf')
fi
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.jsonnet' '*.libsonnet' | { grep -vE "^$(git ls-files --deleted)$" || true; } )
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.java' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.kt' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
  files=$(find "$@" -name '*.kt')
fi
bin=$(rlocation {{ktfmt}})
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.scala' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.go' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
  files=$(find "$@" -name '*.go')
fi
bin=$(rlocation {{gofmt}})
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.sh' '*.bash' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.swift' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
...
if [ "$#" -eq 0 ]; then
  files=$(git ls-files --cached --modified --others --exclude-standard '*.proto' | { grep -vE "^$(git ls-files --deleted)$" || true; })
else
